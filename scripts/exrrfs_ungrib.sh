...
if [[ -s ${outfile} ]]; then
  if [[ -z "${ENS_INDEX}" ]]; then
    ${cpreq} ${DATA}/${outfile} ${UMBRELLA}/ungrib_${TYPE}/
    if [[ "${TYPE}" == "lbc" ]] && [[ ! -d ${UMBRELLA}/ungrib_ic  ]]; then
    ...
    # link ungrib_lbc to ungrib_ic so that ic tasks can run and generate init.nc
      ln -snf ${UMBRELLA}/ungrib_lbc ${UMBRELLA}/ungrib_ic
    fi
  else
    ${cpreq} ${DATA}/${outfile} ${UMBRELLA}/mem${ENS_INDEX}/ungrib_${TYPE}/
  fi
else
